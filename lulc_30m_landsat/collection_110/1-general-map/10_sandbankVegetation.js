...
var soilMask = ee.Image(0).byte().paint({ featureCollection: soilVector, color: 1 })
                  .rename('soil_mask')
                  .reproject({ crs: referenceProjection, scale: 30 })
                  .clip(classificationInput.geometry()).unmask(0).byte();

// HAND (Height Above Nearest Drainage)
var hand = ee.Image('MERIT/Hydro/v1_0_1').select('hnd').reproject({crs: 'EPSG:4674', scale: 30});
...
var referenceMosaic = ee.ImageCollection(mosaicAsset)
    ...
    .filter(ee.Filter.inList('satellite', ['l8']))
    .filter(ee.Filter.gte('year', 2018))
    .filter(ee.Filter.lte('year', 2022))
    .median();

// Spectral predictor bands
var spectralBands = ['red_median', 'nir_median', 'swir1_median', 
...
var sandbankPolygons = ee.FeatureCollection ("projects/ee-ipam-cerrado/assets/Collection_11/sample/manual/collected_samples_herbaceous_sandbank"); 
var nonSandbankPolygons = ee.FeatureCollection ("projects/ee-ipam-cerrado/assets/Collection_11/sample/manual/collected_samples_no-herbaceous_sandbank");
